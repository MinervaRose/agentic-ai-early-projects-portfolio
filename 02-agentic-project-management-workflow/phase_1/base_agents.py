...
class DirectPromptAgent:
    """
    A simple agent that forwards the user prompt directly to the LLM
    and returns the model's response without any additional context.
    """
...
class AugmentedPromptAgent:
    """
    An agent that responds according to a predefined persona.
    """
    ...
    def __init__(self, openai_api_key, persona):
        ...
        openai_api_key : str
            OpenAI API key.
        persona : str
            Description of the persona the agent should adopt.
        """
            ...
            messages=[
                # TODO: 3 - Add a system prompt instructing the agent to assume the defined persona and explicitly forget previous context.
                {
                    ...
                    "content": (
                        ...
                        "Forget all previous context and respond strictly as this persona."
                    ),
                },
...
class KnowledgeAugmentedPromptAgent:
    """
    An agent that responds based on a combination of a persona and
    explicit knowledge supplied at initialization time.
    """
    ...
    def respond(self, input_text):
        ...
        response = client.chat.completions.create(
            model="gpt-3.5-turbo",
            messages=[
                # TODO: 2 - Construct a system message including persona & knowledge instructions.
...
class EvaluationAgent:
    """
    Agent that evaluates and iteratively refines the response from a worker agent
    according to a set of evaluation criteria.
    """
    ...
    def evaluate(self, initial_prompt):
        """
        Manage interactions between the worker agent and the evaluator in order to
        obtain a response that satisfies the evaluation criteria.
...
class RoutingAgent:
    """
    Agent capable of routing user prompts to the most appropriate specialized
    agent based on semantic similarity between the prompt and agent descriptions.
    """
    ...
    def get_embedding(self, text):
        """
        Get the embedding for a given piece of text.
        """
        client = OpenAI(
            base_url="https://openai.vocareum.com/v1",
            api_key=self.openai_api_key,
    ...
    def route(self, user_input):
        """
        Route the user input to the most appropriate agent based on cosine
        similarity between the input embedding and each agent description.
        """
        ...
        for agent in self.agents:
            ...
            agent_emb = self.get_embedding(description) if description else None
...
class ActionPlanningAgent:
    """
    Agent that takes a natural-language description of a task and
    extracts a clear list of action steps using its knowledge.
    """
    ...
    def extract_steps_from_prompt(self, prompt):
        """
        Use the LLM to extract action steps from the prompt based on the agent's knowledge.
        ...
        for line in raw_lines:
            ...
            if not stripped:
                continue
            # Remove common list markers such as "1. ", "- ", "* ", etc.
